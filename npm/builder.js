...
Builder.prototype.rebuild = function (callback) {
  ...
  async.series([
    ...
    callback => builder.patchAsciidoctorCore(callback), // patch Asciidoctor core.
...
Builder.prototype.compileExamples = function (callback) {
  ...
  fs.writeFileSync(path.join(this.examplesBuildDir, 'asciidoctor_example.js'), opalBuilder.build('examples/asciidoctor_example.rb').toString(), 'utf8');
  fs.writeFileSync(path.join(this.examplesBuildDir, 'userguide_test.js'), opalBuilder.build('examples/userguide_test.rb').toString(), 'utf8');
  callback();
};
...
Builder.prototype.compile = function (callback) {
  ...
  fs.writeFileSync('build/asciidoctor-lib.js', opalBuilder.build('asciidoctor').toString(), 'utf8');
